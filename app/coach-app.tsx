...
const COACH_NEXT_STEPS = [
  "Athlete roster and invitations",
  "Shared team training calendars",
  "Coach notes and workout feedback",
];
...
export default function CoachApp() {
  ...
  useEffect(() => {
    ...
    if (profile.accountType !== "coach") {
      router.replace(profile.accountType === "team_runner" ? "/team-app" : "/(tabs)");
    }
    ...
    >
      ...
      >
        ...
        <Text style={{ color: colors.subtext, fontSize: 15, lineHeight: 23, marginTop: 10 }}>
          {displayName}, your account is now routed into the coach experience. This Phase 1 shell confirms coach mode is
          active and gives us the right place to add athlete and team tools next.
        </Text>
      ...
      <SectionCard title="What this shell confirms" colors={colors}>
        <BulletLine text="You are signed in and staying authenticated as a coach." colors={colors} />
        <BulletLine text="Your `account_type` is being used for post-auth routing." colors={colors} />
        <BulletLine text="You are no longer being dropped into the solo runner tabs." colors={colors} />
      </SectionCard>

      <SectionCard title="Coming next" colors={colors}>
        {COACH_NEXT_STEPS.map((item) => (
